...
CINDER_APP_BASIC(SketchPadCoffee, RendererGl(4)); 
...
void SketchPadCoffee::prepareSettings(Settings *settings)
{
	// load config
    try {
...
void SketchPadCoffee::setup() 
{
	LOG_INFO("SketchPadCoffee::setup");	
    ...
    context.add(new fk::script::Date());
    ...
    string script = config.gets("script", "$APP/Contents/Resources/js/main.js");
...
void SketchPadCoffee::reload(string script)
{
    isEnabled = true;
    reloadTimer = 0;
...
void SketchPadCoffee::shutdown()
{
}


void SketchPadCoffee::update() 
{
    // 'Live Coding'
...
void SketchPadCoffee::keyDown(KeyEvent event)
{
...
void SketchPadCoffee::mouseDown(MouseEvent event)
{
    SendMouseEvent(context, isEnabled, sketch, "mouseDown", event);
}

void SketchPadCoffee::mouseUp(MouseEvent event)
{
    SendMouseEvent(context, isEnabled, sketch, "mouseUp", event);
}

void SketchPadCoffee::mouseMove(MouseEvent event)
{
    SendMouseEvent(context, isEnabled, sketch, "mouseMove", event);
}

void SketchPadCoffee::mouseDrag(MouseEvent event)
{
